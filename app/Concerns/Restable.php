...
trait Restable
{
     ...
     *
     * @return JsonResponse The JSON-response with the paginated results
     */
     ...
     */
    public function respondWithMessage($data, $message, $headers = [])
    {
        $data = array_merge(
            $data, [
                'success' => [
     ...
     *
     * @param string $message The given message
     * @param null $status The given status
     * @param array $headers The headers that should be send with the JSON-response
     *
     * @return JsonResponse The JSON-response with the error message
     */
    public function respondWithError($message, $status = null, $headers = [])
    {
        return $this->respond(
            [
                'error' => [
     ...
     * @return JsonResponse The JSON-response with the error code
     */
    protected function respondUnprocessableEntity($message = 'Unprocessable', $status = null, $headers = [])
    {
        $this->setStatusCode(IlluminateResponse::HTTP_UNPROCESSABLE_ENTITY);

        return $this->respondWithError($message, $status, $headers);
    }
     ...
     * @return JsonResponse The JSON-response with the error message
     */
    protected function respondInternalError($message = 'Internal Server Error', $headers = [])
    {
